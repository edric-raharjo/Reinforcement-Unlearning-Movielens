...
# ---------------------------------------------------------------------------
def compute_split(seed):
    """Mirrors main script exactly: set_seed → shuffle sample_users → split."""
    set_seed(seed)
    arr      = np.array(sample_users)
    np.random.shuffle(arr)
    # Main script uses pilot_ratings.nunique() for split size, not pilot_ratings_all
    n_forget = int(np.round(FORGET_PERCENTAGE / 100 * pilot_ratings["user_id"].nunique()))
    return arr[:n_forget].tolist(), arr[n_forget:].tolist()

forget_a, retain_a = compute_split(SEED)
forget_b, retain_b = compute_split(SEED)
forget_c, retain_c = compute_split(SEED)
...
n_forget = int(np.round(FORGET_PERCENTAGE / 100 * n_total))
record(len(forget_a) == n_forget,
       f"Forget size correct: {len(forget_a)} == {n_forget} ({FORGET_PERCENTAGE}% of {n_total:,})")
